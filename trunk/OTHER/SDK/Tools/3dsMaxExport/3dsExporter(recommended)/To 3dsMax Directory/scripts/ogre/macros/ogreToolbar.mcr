...
(
	-- create a floater
	OgreExportFloater = newRolloutFloater "Ogre Exporter (NeoAxis adapted)" 280 800 ;
...
--!!!!!betauser		
		checkbox deleteXMLFiles "Delete xml files after export" pos:[8,42] width:255 height:22 checked:true
...
--					Options.flipYZ= true ;					
				...
				(
					...
					(
						...
						(
							exportMaterial mat (editFilename.text + ".material");
						)
					)
				)
...
--!!!!!!!betauser				
					...
					--(
					xmlConverterPath = ((getdir #maxroot) + "OgreXMLConverter\\OgreXMLConverter.exe");
				
					if (not g_MAX) then
					(
						OptionConv = "";
--!!!!!!betauser			mediaPath = getINISetting ((getDir #scripts) + "\\ogre\\ogreScript.ini") "Directories" "MediaPath"
...
--!!!!!!!betauser
							file = createFile "c:\\tempOgreExporter.bat";
							format ("\"" + xmlConverterPath + "\" " + OptionConv + " \"" + editFilename.text + ".mesh.xml\" \"" + editFilename.text + ".mesh\"") to: file;
							close file;
							DOSCommand "c:\\tempOgreExporter.bat";
							deleteFile "c:\\tempOgreExporter.bat";
							deleteFile "c:\\OgreXMLConverter.log";
--							DOSCommand ("\"" + xmlConverterPath + "\" " + OptionConv + " \"" + editFilename.text + ".mesh.xml\" \"" + editFilename.text + ".mesh\"") ;
--							shellLaunch xmlConverterPath (OptionConv + " \"" + editFilename.text + ".mesh.xml\" \"" + editFilename.text + ".mesh\"") ;
--							DOSCommand (OgreExportOptions.editXMLconverter.text + OptionConv + " \"" + editFilename.text + ".mesh.xml\" \"" + editFilename.text + ".mesh\"") ;

--!!!!!betauser
							if (OgreExportOptions.deleteXMLFiles.checked) then
							(
								DOSCommand ("del \"" + editFilename.text + ".mesh.xml\"");
							)
							
--!!!!!!betauser				DOSCommand ("copy \"" + editFilename.text + ".mesh\" \"" + mediaPath + "\"") ;
						)
						if (exportingSkelDone) then 
						(
--!!!!!!!betauser
							file = createFile "c:\\tempOgreExporter.bat";
							format ("\"" + xmlConverterPath + "\" " + OptionConv + " \"" + editFilename.text + ".skeleton.xml\" \"" + editFilename.text + ".skeleton\"") to: file;
							close file;
							DOSCommand "c:\\tempOgreExporter.bat";
							deleteFile "c:\\tempOgreExporter.bat";
							deleteFile "c:\\OgreXMLConverter.log";
--							shellLaunch xmlConverterPath  (" \"" + editFilename.text + ".skeleton.xml\" \"" + editFilename.text + ".skeleton\"") ;
--							DOSCommand (OgreExportOptions.editXMLconverter.text + " \"" + editFilename.text + ".skeleton.xml\" \"" + editFilename.text + ".skeleton\"") ;
--!!!!!!betauser			DOSCommand ("copy \"" + editFilename.text + ".skeleton\" \"" + mediaPath + "\"") ;						

--!!!!!betauser
							if (OgreExportOptions.deleteXMLFiles.checked) then
							(
								DOSCommand ("del \"" + editFilename.text + ".skeleton.xml\"");
							)
						)
--							messageBox "Export completed.";
--!!!!!!betauser
						print (xmlConverterPath + " " + OptionConv + " \"" + editFilename.text + ".mesh.xml\" \"" + editFilename.text + ".mesh\"") ;
...
----!!!!!!betauser	)
	...
	(
		...
		label label17 "+ additional NeoAxis Group changes" align:#center ;
	)
